...
import ini_zoom_info from "./ini_zoom_info.js";
import toggle_grid_lines from "../matrix/toggle_grid_lines.js";
export default (function two_translate_zoom(cgm, pan_dx, pan_dy, fin_zoom) {
    ...
    if (!params.viz.run_trans) {
        ...
        ['row', 'col'].forEach(function (inst_rc) {
            const selection = d3.select(params.root + ' .' + inst_rc + '_dendro_crop_buttons');
            // Fixes case when there are no clusters defined
            if (!selection[0] || selection[0][0] === null)
                return;
        ...
        d3.select(params.root + ' .col_cat_container')
            .transition()
            .duration(search_duration)
            .attr('transform', ' scale(' + zoom_x + ',' + 1 + ')' + 'translate(' + [pan_dx, 0] + ')');
        ...
        /////////////////////////////////////////
        if (d3.select(params.root + ' .row_label_zoom_container text').empty() ===
            false) {
        ...
        d3.select(params.root + ' .row_dendro_icons_group').attr('transform', 'translate(' +
            [0, 0 + center_y] +
            ')' +
            ' scale(' +
            zoom_x +

...
module.exports = function show_visible_area(
  ...
  if (make_all_rows === false) {
    missing_rows = params.viz.viz_nodes.row.filter(
      (x) => !params.viz.viz_nodes.curr_row.includes(x)
    );
  } else {
  ...
  if (zooming_stopped === true) {
    ...
    if (new_ds_level >= 0) {
      ...
      d3.selectAll(params.root + " .ds" + String(new_ds_level) + "_row").each(
        function (d) {
          if (!params.viz.viz_nodes.row.contains(d.name)) {
  ...
  if (new_ds_level === -1) {
    if (zooming_out === false || zooming_stopped) {
      ...
      // find missing labels
      var missing_row_labels = params.viz.viz_nodes.row.filter(
        (x) => !visible_row_labels.includes(x)
      );

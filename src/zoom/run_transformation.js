...
import resize_label_val_bars from "./resize_label_val_bars.js";
import zoom_crop_triangles from "./zoom_crop_triangles.js";
import get_previous_zoom from "./get_previous_zoom.js";
import run_when_zoom_stopped from "./run_when_zoom_stopped.js";
import check_zoom_stop_status from "./check_zoom_stop_status.js";
export default (function run_transformation(cgm) {
    const params = cgm.params;
    const zoom_info = params.zoom_info;
    const prev_zoom = get_previous_zoom(params);
    d3.select(params.root + ' .clust_group').attr('transform', 'translate(' +
    ...
    d3.select(params.root + ' .row_cat_container').attr('transform', 'translate(' +
        [0, zoom_info.trans_y] +
        ') scale( 1,' +
        zoom_info.zoom_y +
        ')');
    d3.select(params.root + ' .row_dendro_container').attr('transform', 'translate(' +
    ...
    d3.select(params.root + ' .col_cat_container').attr('transform', 'translate(' +
        [zoom_info.trans_x, 0] +
        ') scale(' +
        zoom_info.zoom_x +
        ',1)');
    d3.select(params.root + ' .col_dendro_container').attr('transform', 'translate(' +

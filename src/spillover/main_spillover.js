import d3 from "d3";
import get_cat_title from "../categories/get_cat_title.js";
...
import make_dendro_crop_buttons from "../dendrogram/make_dendro_crop_buttons.js";
export default (function Spillover(cgm) {
    ...
    d3.select(viz.root + ' .col_container')
        ...
        .attr('transform', 'translate(' + viz.clust.dim.width + ',' + viz.norm_labels.width.col + ')');
    ...
    d3.select(viz.viz_svg)
        ...
        const tmp_top = viz.norm_labels.width.col + viz.norm_labels.margin.top - viz.uni_margin;
        return 'translate(' + tmp_left + ', ' + tmp_top + ')';
    });
    let x_offset = viz.clust.margin.left + viz.clust.dim.width + viz.uni_margin;
    let y_offset = viz.norm_labels.margin.top +
    ...
    if (viz.show_categories.col) {
        d3.select(viz.viz_svg)
            ...
            const inst_y = y_offset + extra_y_room * viz.cat_room.symbol_width * inst_cat;
